function [f,Phasor,Cn,theta] = f_FourierTrapezeArrondi(A,T0,a,tm,td,Fend,DCb,varagin)
...
Cn(1)=A*a+DCb;
theta(1)=0;
% F(ret(tau*t) = tau*sinc(n*f(2)*tau)
Cn(2:end)=(A./(pi*n)).*sqrt((sinc(n*Bm)).^2+(sinc(n*Bd)).^2-2*sinc(n*Bm).*sinc(n*Bd).*cos(pi*n*(2*a))).*sinc(f(2:end)*tau);
